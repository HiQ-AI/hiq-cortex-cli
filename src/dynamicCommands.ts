 ...
 * drift. `aggregate_datasets` becomes `hiq-cortex aggregate-datasets` with real flags
...
/** `aggregate_datasets` → `add-exchange`, `epd_peer_benchmark` → `epd-peer-benchmark`. */
 ...
 *  text stays available via `hiq-cortex describe <tool>`. */
...
function parseJsonFlag(flag: string, value: string): unknown {
  try {
    ...
    throw new CortexClientError("validation", `--${flag} must be valid JSON: ${String(e)}`);
  }
}
